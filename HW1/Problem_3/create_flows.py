            ...
            for idx in range(len(Path_d['receive'])):
                if Path_d['receive'][idx] == sw_index + 1:
                    try:
                        ...
                        if idx == 0:
                            switch_SFlow_Adder(sw_index + 1, flow_n, dst_mac_addr, dst_net_addr, outPort)
                        else:
                            switch_Flow_Adder(sw_index + 1, flow_n, dst_net_addr, outPort)

                        flow_n += 1
                    except:
                        switch_Flow_Adder(sw_index + 1, flow_n, dst_net_addr, 1)
                        flow_n += 1
                    break
