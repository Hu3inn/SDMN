...
def topol():
    ...
    switches = [net.addSwitch('s%s' % s,
                              mac='00:00:00:00:0{}:00'.format(s),
                              protocols='OpenFlow13') for s in range(1, NumOfNodes + 1)]
    ...
    for j in range(len(mat)):
        for i in range(len(mat[j])):
            L_cost = mat[j][i]
            if (L_cost != 0) and ('{}--{}'.format(i + 1, j + 1) not in Llist):
                Llist.append('{}--{}'.format(j + 1, i + 1))
